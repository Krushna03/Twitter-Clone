...
const TwitterLayout: React.FC<TwitterLayoutProps> = (props) => {
  ...
  const sidebarMenuItems: TwiiterSidebarButton[] = useMemo(() => [
    {
      title: "Home",
      icon: <BiHomeCircle />,
      link: "/"
  ...
  return (
    <div>
      <div className="grid grid-cols-12 h-screen w-screen px-2 sm:px-10 lg:px-56">
            ...
            <ul>
              {sidebarMenuItems.map((item) => (
                <li
                  key={item.title}
                >
                  <Link href={item.link} className="flex justify-center sm:justify-start items-center gap-0 sm:gap-4 hover:bg-[#1c1c1a] rounded-full p-3 sm:px-5 sm:py-3 w-fit cursor-pointer mt-2">
        ...
        {/* Right Section */}
        <div className="hidden sm:block sm:col-span-3 lg:col-span-3 m-4">
          {!user ? (
            <div className='px-3'>
            ...
            <div className='px-4 py-3 bg-slate-900 rounded-lg'>    
              <h1 className='my-2 text-xl'>User you may know</h1>
              {
                user?.recommendedUsers?.map((el) => (
                  <div className='flex items-center gap-3 mt-5' key={el?.id}>
                    {
                      el?.profileImage && (
                        <Image
                          src={el?.profileImage}
                    ...
                    <div>
                      ...
                      <Link href={`/${el?.id}`} className="bg-white text-black text-xs sm:text-sm font-semibold py-1 px-3 sm:py-1 sm:px-5 rounded-lg hover:opacity-90 transition cursor-pointer w-full">
                        View
                      </Link>
                    </div>
                  </div>
